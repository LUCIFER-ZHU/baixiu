<?php 
  // 侧边栏只是页面一部分，所以引入fn.php和session_start()都做过了

  //在侧边栏 显示当前登录用户的头像和昵称
  // 1-如果用户能看到侧边栏，说明用户肯定登录过
  // 2-在用户登录成功时， 把用户id存到session中了
  // 3-可以去session中获取用户id
  $id = $_SESSION['user_id'];
  // 4-根据id去数据库中获取对应的用户信息
...
<div class="aside">
    ...
    <ul class="nav">
      ...
      <li class="<?php echo $isSet? 'active': '' ?>">
        <a href="#menu-settings" class="<?php echo $isSet? '': 'collapse' ?>" data-toggle="collapse">
